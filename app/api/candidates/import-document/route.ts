...
- photo_description: 書類に証明写真・顔写真が含まれている場合、その人物の外見の説明（例: "スーツを着た男性、30代前半"）。写真がない場合はnull
- has_photo: 書類に候補者の写真が含まれているかどうか（true/false）
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Second API call for detailed photo extraction (PDF and image files only)
    if (isDocumentType(mediaType) || isImageType(mediaType)) {
      try {
        const photoMessage = await anthropic.messages.create({
          model: 'claude-sonnet-4-20250514',
          max_tokens: 1024,
          messages: [
            {
              role: 'user',
              content: isDocumentType(mediaType)
                ? [
                    {
                      type: 'document' as const,
                      source: {
                    ...
                    {
                      type: 'text' as const,
                      text: 'この書類に候補者の証明写真・顔写真が含まれている場合、その写真部分だけを詳細に説明してください。写真がない場合は \'no_photo\' と返してください。',
                    },
                  ]
                : [
                    {
        ...
        const photoResponseText = photoMessage.content
          .filter((block): block is Anthropic.TextBlock => block.type === 'text')
          .map((block) => block.text)
          .join('')

        if (photoResponseText.trim() !== 'no_photo') {
          extracted.photo_description = photoResponseText.trim()
          extracted.has_photo = true
        }
      } catch (photoErr) {
        console.error('Photo extraction failed:', photoErr)
        // Don't fail the request if photo extraction fails
      }
    }
    ...
    const { data: candidate, error: dbError } = await db
      ...
      .insert({
        ...
        metadata: {
          photo_description: extracted.photo_description || null,
          has_photo: extracted.has_photo || false,
        },
      })
      .select()
      .single()
    ...
    // Upload the original document to storage for photo reference
    try {
      const storagePath = `candidates/${candidate.id}/documents/${file.name}`
      ...
      if (urlData?.publicUrl) {
        await db.from('candidates').update({
          metadata: {
            ...
            photo_description: extracted.photo_description,
            has_photo: extracted.has_photo,
          }
        }).eq('id', candidate.id)
      }
    } catch (storageErr) {
